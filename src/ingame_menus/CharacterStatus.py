...
class CharacterStatus():
    ...
    def __init__(self, game):
        ...
        self.screen_position = self.game.resolution[0]//2-self.surface.get_width()//2, self.game.resolution[1]//2-self.surface.get_width()//2

        #Builtin text that we will change later 
        self.character_name = Text(self.surface, 160, 125, "Personnage", path.join(fonts_folder, "alegreya.ttf"), "#2C2A27", 50, True)
        self.future_text = Text(self.surface, 160, 300, "[Les stats du personnage]", path.join(fonts_folder, "alegreya.ttf"), "#2C2A27", 14, True)
    ...
    def blur_surface(self, surface):
        ...
        for i in range(1, 4):
            surface = pg.transform.smoothscale(surface, (self.game.resolution[0]*i, self.game.resolution[1]*i))
            surface = pg.transform.smoothscale(surface, (self.game.resolution[0]//i*2, self.game.resolution[1]//i*2))
